...
from CBIR.evaluate import distance, evaluate_class
  ...
  def make_samples(self, db, verbose=True):
    ...
    try:
      ...
      for d in data.itertuples():
        d_img, d_url, d_cls = getattr(d, "img"), getattr(d, "url"), getattr(d, "cls")
        d_hist = self.histogram(os.path.join(DATASETS_PATH, d_cls, d_img+'.jpg'), type=h_type, n_bin=n_bin, n_slice=n_slice)
        if d_hist is None:
          continue
        samples.append({
                        'img':  d_img, 
...
if __name__ == "__main__":
  ...
  hist = color.histogram(os.path.join(DATASETS_PATH, data['cls'][0], data['img'][0]+'.jpg'), type='global')
